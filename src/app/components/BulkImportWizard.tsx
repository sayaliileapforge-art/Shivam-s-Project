...
import {
  ...
  Download, RefreshCw, ArrowRight, ArrowLeft, Database, Eye, X,
} from "lucide-react";
...
interface Props {
  ...
  onComplete: (fields: ProjectDataField[], records: ProjectDataRecord[]) => void;
  /** If provided, a Cancel button is shown */
  onCancel?: () => void;
}

// ─── Component ────────────────────────────────────────────────────────────────

export function BulkImportWizard({ projectId, category, onComplete, onCancel }: Props) {
  ...
  const reset = () => {
    ...
    setMapping({}); setErrors([]); setImportDone(null);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          <h2 className="text-base font-semibold">Bulk Data Import</h2>
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          {onCancel && (
            <Button variant="ghost" size="sm" className="gap-1.5 text-xs" onClick={onCancel}>
        ...
        <Card className="shadow-sm">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-base">
              <FileSpreadsheet className="h-4 w-4 text-success" /> Upload File
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div
            ...
            >
              ...
              <p className="text-sm text-muted-foreground">.xlsx, .xls, or .csv files up to 10 MB</p>
            </div>
            <input
        ...
        <div className="space-y-4">
          ...
          {unmappedRequired.length > 0 && (
            <p className="text-sm text-destructive flex items-center gap-2">
              <AlertCircle className="h-4 w-4" /> Map all required (*) fields before continuing.
            </p>
          )}
        ...
        <div className="space-y-4">
          <Card className="shadow-sm">
            <CardHeader>
              <div className="flex items-center justify-between">
                <CardTitle className="flex items-center gap-2 text-base">
                  <Eye className="h-4 w-4" /> Data Preview (first 10 rows)
                </CardTitle>
        ...
        <Card className="shadow-sm">
          <CardContent className="p-10 text-center space-y-4">
            ...
            <div className="flex gap-2 justify-center flex-wrap">
              ...
              {onCancel && (
                <Button onClick={onCancel} className="gap-2">Done</Button>
              )}
            </div>
          </CardContent>
        </Card>
